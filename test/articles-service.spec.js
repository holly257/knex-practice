...
describe('Articles service object', function() {
    ...
    context(`Given 'blogful_articles' has data`, () => {
        ...
        it(`updateArticle() updates an article from the 'blogful_articles table`, () => {
            const idOfArticleToUpate = 3
            const newArticleData = {
                title: 'updated title',
                content: 'updated content',
                date_published: new Date(),
            }
            return ArticlesService.updateArticle(db, idOfArticleToUpate, newArticleData)
                .then(() => ArticlesService.getById(db, idOfArticleToUpate))
                .then(article => {
                    expect(article).to.eql({
                        id: idOfArticleToUpate,
                        ...newArticleData,
                    })
                })
        })
    ...
    context(`Given 'blogful_articles' has no data`, () => {
        ...
        it(`insertArticle() inserts a new article and resolves the new articles with an 'id'`, () => {
            ...
            return ArticlesService.insertArticles(db, newArticle)
                .then(actual => {
                    expect(actual).to.eql({
                        id: 1,
